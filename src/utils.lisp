...
(sera:-> mix-with-mask ((simple-array bit)
                        (simple-array bit)
                        (simple-array bit))
         (values (simple-array bit) &optional))
(defun mix-with-mask (m u v)
  (declare (optimize (speed 3)))
  (unless (and (equalp (array-dimensions m)
                       (array-dimensions u))
               (equalp (array-dimensions m)
                       (array-dimensions v)))
    (error "M, U and V must have same dimensions"))
  (let ((result (make-array (array-dimensions m)
                            :element-type 'bit)))
    (map-into (sb-ext:array-storage-vector result)
              (lambda (m u v)
                (if (zerop m) v u))
              (sb-ext:array-storage-vector m)
              (sb-ext:array-storage-vector u)
              (sb-ext:array-storage-vector v))
    result))
